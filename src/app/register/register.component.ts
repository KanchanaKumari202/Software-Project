...
export class RegisterComponent implements OnInit {
    ...
    this.registrationForm = this.formBuilder.group(
      {
          ...
          nic: ['', [Validators.required, Validators.maxLength(11), Validators.minLength(10), Validators.pattern('([0-9]{9}(v|V)|[0-9]{11})')]],
  ...
  uploadPdfToStore(){
    return new Promise((resolve,reject)=>{
      this.pdfUploadService.uploadPdfToStorage(this.files,'CVPdf').then(
        res=>{
      ...
      res=>{
       ...
       this.applicant.evaluated = false;
        if (this.attach != null) {
          ...
          // alert(fd)
          this.apiService.uploadAttachment(fd).subscribe(
            reply => {
              // alert("Applicant is going to be saved "+JSON.stringify(this.applicant))
              this.apiService.registerUser(this.applicant);
              alert("Your applicant record is successfully saved!! Stay alert with emails to check your cv is referred!")
            }
          )
        } else {
